    ...
    <div class="invoice" style="text-align: center;">
        <div style="padding-top: -30px">
            ...
            <div style="margin: 10px 0px; font-size:1.5em;" >
                <div  style="border: 2px solid #0A77BA;border-radius: 10px; width:30%; margin:auto;">
                    <div style="background-color: #0A77BA;color: #eceff4; margin:auto;">Sales Return invoice <span style="font-family: dejavusans"></span> </div>
                 </div>
            </div>
            @if ($setup->vat_number)
            <div>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td colspan="1">
                        {{$sale->salereturndetails->sum('return_qty') }}
                    </td>
                    <td>
                        {{$sale->salereturndetails->sum('already_return_qty') }}
                    </td>
                    <td> {{($sale->salereturndetails->sum('sale_price')) }}</td>
                
                    <td> {{($sale->salereturndetails->sum('vat_amount')) }}</td>
                    <td></td>
                    <td> {{($sale->salereturndetails->sum('discount_amount')) }}</td>
                    <td>
                        {{($sale->salereturndetails->sum('total_price')) }}
                    </td>
                </tr>
            ...
            <tfoot>
                ...
                <tr>
                    <td colspan="6" style="text-align: center">Payment Method: {{@$sale->payment_method}}</td>
                    <td colspan="2" style="text-align: right">Grand Total</td>
                    <td> {{ (@$sale->grand_total) }}</td>
                   </tr>
                <tr>
                    <td colspan="6" style="text-align: center"> @if (@$sale->payment_method=='Bkash' ||@$sale->payment_method=='Nagad'|| @$sale->payment_method=='Rocket' )
