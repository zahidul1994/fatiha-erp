...
@section('title', 'Create Damage Products')
@push('css')
      ...
      <div class="card">
        <div class="card-body">
          <div class="d-flex align-items-center">
           <h3 class="text-danger">You Can Not Edit Or Delete.Be Carefully </h3> <a href="{{ route(Request::segment(1) . '.damage-products.index') }}" class="btn btn-primary btn-sm ms-auto">Back</a>
          </div>
          <div class="row">
            @include('partial.formerror')
            {!! Form::open(['route' => Request::segment(1) . '.damage-products.store', 'method' => 'POST', 'id' =>
            'formReset']) !!}
            @include('backend.common.damage_products.form')
  ...
  $(document).ready(function () {
    ...
    source: function (request, response) {
        if (!$('#shop_id').val()) {
               ...
               $.ajax({
                type: 'POST',
               url: "{{ URL(Request::segment(1).'/find-damage-product') }}",
               data: {
                term: request.term,
                shop_id: $('#shop_id').val(),
            },
         ...
         if (ui.item.id !== 0) {
            ...
            $counter = $("#itemlist tr").length;
                ...
                $itemlist.append( '<tr>\
                        ...
                        <td style="width:120px;"><input class="form-control input-sm text-end" placeholder="0.00" tabindex="10" name="product_price[]" onblur="getQty(' + i + ',this);" data-cell="P'+i+'" value="'+row.price+'" readonly data-format=""><i class="text-black-50">&nbsp;</i></td>\
                        <td style="width:120px;"><input class="form-control input-sm text-end pquentity" required  type="number" step="any" min=1 max="'+row.stock+'" required id="id'+row.id+'" data-format="" data-format="0[.]00" onblur="getQty(' + i + ',this);"   name="product_quantity[]"  data-cell="Q'+i+'"><i class="text-black-50">'+'S-' +row.stock+'</i></td> \
                        <td style="width:80px;"><input type="number" step="any" min=0 max=999999999 class="form-control input-sm text-end" readonly onblur="getVat(' + i + ',this);" placeholder="0.00" name="product_vat[]" data-cell="T'+i+'" value="'+row.vat+'" data-format="0[.]0000"><i class="text-black-50">&nbsp;</i></td>\
                        ...
                        <td style="width:160px;"><input type="number" readonly step="any" min=1 max=999999999 class="form-control input-sm text-end" placeholder="0.00" name="product_total_price[]"  data-cell="F'+i+'" data-format="0[.]00" data-formula="(Q'+i+')*(P'+i+')"><i class="text-black-50">&nbsp;</i></td>\
